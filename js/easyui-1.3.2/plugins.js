...
(function($) {
	...
	function refresh(_1b) {
		...
		function _20(_21) {
			...
			if ($.parser) {
				var $target = $(_1b);
				_1d.onBeforeParse.call($target, $target);
				$.parser.parse($(_1b));
				_1d.onParse.call($target, $target);
			}
		}
		;
	}
	...
	function _1e(_22) {
		...
		t.find(".chooser-f").each(function() {
		...
		refresh : function(jq, param) {
			return jq.each(function() {
				...
				if (typeof param == "string" ) {
					...
					$(this).attr("page-params", true).data("page-params", options.params);
				}
				refresh(this);
			});
		},
	...
	$.fn.panel.defaults = {
		...
		extractor : function(html) {
			...
			text = Page.getEvalHtml(text, Page.getParams(this));
			return text;
		},
		onLoad : function() {
		},
...
(function($) {
	...
	;
	function _open(title, content, buttons, options) {
		var win = $("<div class=\"messager-body\"></div>").appendTo("body");
		win.append(content);
		if (buttons) {
			var tb = $("<div class=\"messager-button\"></div>").appendTo(win);
			for ( var name in buttons) {
				$("<a></a>").attr("href", "javascript:void(0)").text(name).css("margin-left", 10).bind("click", eval(buttons[name])).appendTo(tb).linkbutton();
			}
		}
		win.window($.extend({
			title : title,
	...
	$.messager = {
		...
		alert : function(_15, msg, _16, fn, options) {
			var _17 = "<div>" + msg + "</div>";
			switch (_16) {
			case "error":
			...
			var win = _open(_15, _17, _18, options);
			return win;
		},
		confirm : function(_19, msg, fn, options) {
			...
			var win = _open(_19, _1a, _1b, options);
			return win;
		},
		prompt : function(_1c, msg, fn, options) {
			...
			var win = _open(_1c, _1d, _1e, options);
		...
		progress : function(_1f) {
			...
			var win = _open(_22.title, _23, null);
		...
		open : function(title, content, buttons, options) {
			var html = "<div>" + content + "</div><div style=\"clear:both;\"/>";
			var proxy = {}, buttons = buttons || {};
			for(var name in buttons ){
				proxy[name] = function(){
					win.window("close");
					buttons[name]();
			...
			var win = _open(title, html, proxy, options);
			return win;
		}
	};
	$.messager.defaults = {
	...
	function _52(_53, ul, _54, _55) {
		...
		function _59(ul, _5c, _5d) {
			...
			for ( var i = 0; i < _5c.length; i++) {
				...
				var texted = _56.formatter ? _56.formatter(_5e) : _5e.text;
				$("<span class=\"tree-title\"></span>").html(texted).appendTo(_5f);
				if (_56.checkbox) {
...
(function($) {
	function _1(_2) {
		...
		var _4 = $("<input type=\"search\" class=\"searchbox-text\">").appendTo(_3);
			...
			$(_35).children("thead").each(function() {
				...
				$(this).find("tr").each(function() {
					...
					$(this).find("th").each(function() {
						var th = $(this);
						var col = $.extend({}, $.parser.parseOptions(this, [ "field", "align", "halign", "order","tip",{
							sortable : "boolean",
							checkbox : "boolean",
							resizable : "boolean"
						}, {
			...
			if (typeof _4a.toolbar == "string") {
				...
				for ( var i = 0; i < _4a.toolbar.length; i++) {
					...
					if( btn.type == "linkbutton" ){
						...
						if( btn.handler ){
							var _50 = $("<button id='" + id + "' href=\"javascript:void(0)\" class='button' type='button' " + disable + ">" + btn.text + "</button>").appendTo(td);
							_50[0].onclick = eval(btn.handler || function() {
							});
						}else{//显示文本
			...
			for ( var i = 0; i < _56.length; i++) {
				...
				for ( var j = 0; j < _58.length; j++) {
					...
					if (col.checkbox) {
						...
						if (col.field) {
							td.attr("field", col.field);
							var tipClass = col.tip ? "datagrid-column-tip" : "";
							td.append("<div class=\"datagrid-cell " + tipClass + "\"><span></span><span class=\"datagrid-sort-icon\"></span></div>");
	...
	function _81(_82, _83) {
		...
		if (_83) {
			...
			for ( var i = 0; i < _86.length; i++) {
				...
				if (col && col.auto) {//修复复合表头出现JS错误的BUG
					_9(_83);
					_85 = true;
				}
			}
		...
		tr.children("td").each(function() {
			...
			if (col && col.editor) {
				...
				if (editor) {
					...
					$.data(cell[0], "datagrid.editor", editorState);
					var event = col.editor.options ? (col.editor.options.event || {}) : {};
					for(var name in event){
						$(editorState.target).off(name + ".datagrid-editor").on(name + ".datagrid-editor", event[name]);
					}
				}
			}
		});
	...
	var _1c9 = {
		render : function(_1ca, _1cb, _1cc) {
			...
			for ( var i = 0; i < rows.length; i++) {
				...
				var _1d1 = _1d0.indexOf(":") != -1 ? "style=\"" + _1d0 + "\"" : "";
				var trClass = _1d0.indexOf(":") != -1 ? "" : _1d0;
				var cls = (i % 2 && opts.striped) ? "class=\"datagrid-row datagrid-row-alt " + trClass + "\"" : "class=\"datagrid-row " + trClass + "\"";
...
//			header.addClass('tabs-header-left tabs-header-right');
	...
	function createTab(container, pp, options) {
		...
		options = options || {};
		var params = options.params || $.eval(pp.attr("params") || "{}");
		pp.attr("page-params", true).data("page-params", params);
		// create panel
...
//		setProperties(container);
	...
	$.fn.tabs.parseOptions = function(target){
		return $.extend({}, $.parser.parseOptions(target, [
			...
			{fit:'boolean',border:'boolean',plain:'boolean',headerWidth:'number',showHeader:'boolean'}
		]));
	};
...
(function($) {
	...
	$.fn.tooltip.parseOptions = function(_26) {
		...
		var _27 = $.extend({}, $.parser.parseOptions(_26, [ "position", "showEvent", "hideEvent", "content",
				{ deltaX : "number", deltaY : "number", showDelay : "number", hideDelay : "number" } ]), { _title : t.attr("title") });
		t.attr("title", "");
		if (!_27.content) {
	...
	$.fn.tooltip.defaults = { position : "bottom", content : null, trackMouse : false, deltaX : 0, deltaY : 0, showEvent : "mouseenter",
